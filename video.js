...
const video_icon = `<i class="ri-movie-2-fill fs-3 text-info me-1"></i>`
...
function list(path, id = '', fallback = false) {
	...
	if (navarray.length > 1) {
		for (var i in navarray) {
			...
			var displayedPathPart = trimmedPathPart.length > 35 ? trimmedPathPart.slice(0, 15) + '...' : trimmedPathPart.slice(0, 35);
